...
(function () {
    function sum(arr, f) {
        ...
        if (typeof f != 'function') f = function f(x) {
            return x;
        };
    ...
    console.log(sum([1, 2, 3], function (x) {
        return x * x;
    }));
    console.log(sum([1, 2, 3], function (x) {
        return Math.pow(x, 3);
...
(function () {
    var dd = new Date(Date.UTC(2016, 4, 27));
    console.log(dd);
    //如果需要在服务器端构建一个具有指定时区（而非手动转换时区）的Date实例，可以使用moment.tz
    //需要引用moment-timezone库
    var moment = require('moment-timezone');
    var d = moment.tz([2016, 3, 27, 9, 19], 'America/Los_Angeles').toDate();
    console.log(d);
...
//展示日期
(function () {
    var d = new Date(Date.UTC(1930, 4, 10, 6, 6, 7));
    console.log(d);
    ...
    console.log('_____________________');
    var d1 = new Date(1996, 2, 1);
    var d2 = new Date(2009, 4, 27);
    var d3 = moment();
    var msDiff = d2 - d1;
    console.log(msDiff.valueOf());
    console.log(d3);
    //将其推至9小时后，再减去3分钟
    d3.add(9, 'hours').subtract(3, 'minutes');
    console.log(d3);
})();
